...
def trans_output(out_mean, out_var, idx_validation, columns):
...
def compute_val_loss(net, val_loader, criterion, sw, epoch):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            print('batch_index day', batch_index)
            st = time()
            ...
            print('decoder_inputs be', decoder_inputs.shape)
            ...
            #XL///
            label_mean = np.squeeze(labels[:,:,0:1,:,:], axis=2)
            label_var = np.squeeze(labels[:,:,1:2,:,:],axis=2)

            predict_length = label_mean.shape[2]  # T
            # encode
            ...
            decoder_input_list_v = [decoder_start_inputs]
            # 按着时间步进行预测
            for step in range(predict_length):
                decoder_inputs = torch.cat(decoder_input_list_v, dim=2)
                predict_output_v = net.decode(decoder_inputs, encoder_output)
                decoder_input_list_v = [decoder_start_inputs, predict_output_v]

            end = time()
            elapsed = end - st
            print('validation time per sample:', elapsed, batch_size, round(elapsed/batch_size,8))
            # print('predict_output m',predict_output.shape)
            # print('labels m', label_mean.shape)
            # print('predict_output v', predict_output.shape)
            # print('labels v', label_var.shape)

            # predict_output_q = predict_output[:, :, 0, :]
            # label_mean_q = label_mean[:, :, 0, :]
            # predict_output_v_q = predict_output_v[:, :, 0, :]
            # label_var_q = label_var[:, :, 0, :]



            loss_m = criterion(predict_output, label_mean)  # 计算误差
            loss_v = criterion(predict_output_v, label_var)
            loss = loss_m + loss_v
            tmp.append(loss.item())
            if batch_index % 100 == 0:
                print('validation batch %s / %s, loss: %.2f' % (batch_index + 1, val_loader_length, loss.item()))

            predict_output_ravel = torch.reshape(predict_output, (-1,1))
            predict_output_v_ravel = torch.reshape(predict_output_v,(-1,1))
            label_mean_ravel = torch.reshape(label_mean, (-1,1))
            label_var_ravel = torch.reshape(label_var, (-1,1))



            e_dist  = eval_dist(predict_output_ravel.cpu().numpy(), predict_output_v_ravel.cpu().numpy(), label_mean_ravel.cpu().numpy(), label_var_ravel.cpu().numpy())
            kl = eval_kl(predict_output_ravel.cpu().numpy(), predict_output_v_ravel.cpu().numpy(), label_mean_ravel.cpu().numpy(), label_var_ravel.cpu().numpy())
            print('e_dist:',e_dist)
            print('kl:',kl)

            # val_len = predict_output_q.shape[0]
            # predict_output_ravel_q = torch.reshape(predict_output_q, (-1, 1))
            # predict_output_v_ravel_q = torch.reshape(predict_output_v_q, (-1, 1))
            # label_mean_ravel_q = torch.reshape(label_mean_q, (-1, 1))
            # label_var_ravel_q = torch.reshape(label_var_q, (-1, 1))
            #
            # query_pre = trans_output(predict_output_ravel_q, predict_output_v_ravel_q, range(val_len), columns)
            # query_true = trans_output(label_mean_ravel_q, label_var_ravel_q, range(val_len), columns)
            # if not os.path.exists(f'./AST_query_data/{time_interval}/{epoch}'):
            #     os.makedirs(f'./AST_query_data/{time_interval}/{epoch}')
            # query_pre_df = pd.DataFrame(query_pre)
            # query_true_df = pd.DataFrame(query_true)
            # query_pre_df.to_csv(f'./AST_query_data/{time_interval}/{epoch}/query_pre.csv', sep='\t', header=False, index=False)
            # query_true_df.to_csv(f'./AST_query_data/{time_interval}/{epoch}/query_true.csv', sep='\t', header=False, index=False)
...
def predict_and_save_results(net, data_loader, data_target_tensor, epoch, _max, _min, params_path, type):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(data_loader):
            ...
            # elapsed = end - st
            # print('test validation time per sample:', elapsed, batch_size, round(elapsed/batch_size,8))
            print('test predict_output m', predict_output.shape)
            print('test labels m', label_mean.shape)
            print('test predict_output v', predict_output_v.shape)
            print('test labels v', label_var.shape)
            ...
            # # end = time()
            # # elapsed = end - st
            # # print('test execution time per sample:', round(elapsed/batch_size, 8))
            ...
            # label_var_ravel = torch.reshape(label_var, (-1, 1))
            #
            # e_dist = eval_dist(predict_output_ravel, predict_output_v_ravel, label_mean_ravel, label_var_ravel)
            # kl = eval_kl(predict_output_ravel, predict_output_v_ravel, label_mean_ravel, label_var_ravel)
            # print('test dataset e_dist:', e_dist)
            # print('test dataset kl:', kl)
...
def load_graphdata_normY_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True, percent=1.0):
    ...
    train_x = train_x[:, :, 0:1, :]
    print('train_xxxxxx', train_x.shape)

    train_target = file_data['train_target']  # (10181, 307, 12)
    print('train_targettttttt', train_target.shape)
    ...
    train_target_part = np.squeeze(train_target[:, :, 0:1, :-1], 2)
    print('train_target_norm', train_target_part.shape)
    train_decoder_input = np.concatenate((train_decoder_input_start, train_target_part), axis=2)  # (B, N, T)
    ...
    train_target_tensor = torch.from_numpy(train_target).type(torch.FloatTensor).to(DEVICE)  # (B, N, T) #XL
    ...
    val_target_part = np.squeeze(val_target[:, :, 0:1, :-1], 2)
    print('val_decoder_input_start, val_target_part', val_decoder_input_start.shape, val_target_part.shape)
    val_decoder_input = np.concatenate((val_decoder_input_start, val_target_part), axis=2)  # (B, N, T)
    ...
    test_target_part = np.squeeze(test_target[:, :, 0:1, :-1], 2)
    test_decoder_input = np.concatenate((test_decoder_input_start, test_target_part[:, :, :-1]), axis=2)  # (B, N, T)
