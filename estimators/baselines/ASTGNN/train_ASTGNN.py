#!/usr/bin/env python
# coding: utf-8
#NB: In order to fit into our setting, the code here is adapted from https://github.com/guoshnBJTU/ASTGNN
import sys
import torch
...
def train_main():
    if (start_epoch == 0) and (not os.path.exists(params_path)):  # 从头开始训练，就要重新构建文件夹
        ...
        raise SystemExit('Wrong type of estimators!')
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # apply estimators on the validation data set
        ...
        for batch_index, batch_data in enumerate(train_loader):
            print('training batch epoch/index', epoch, '/',batch_index)
            ...
            output_m, output_v = net(encoder_inputs, decoder_inputs)

            loss_m = criterion(output_m, label_mean)
            loss_v = criterion(output_v, label_var)
            loss = loss_m + loss_v
            loss.backward()
            optimizer.step()
    ...
    print('apply the best val estimators on the test data set ...', flush=True)
