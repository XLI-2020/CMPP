...
class TemporalPositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout, max_len, lookup_index=None):
        ...
        # This is typically used to register a buffer that should not to be considered a estimators parameter.
...
class EncoderDecoder(nn.Module):
    def __init__(self, encoder, decoder, src_dense, trg_dense, generator_m, generator_v,DEVICE):
        super(EncoderDecoder, self).__init__()
        self.encoder = encoder
        self.decoder = decoder
        self.src_embed = src_dense
        self.trg_embed = trg_dense
        self.prediction_generator_m = generator_m
        self.prediction_generator_v = generator_v
        self.to(DEVICE)
    ...
    def forward(self, src, trg):
        ...
        return self.decode(trg, encoder_output), self.decode_v(trg, encoder_output)
    ...
    def decode(self, trg, encoder_output):
        return self.prediction_generator_m(self.decoder(self.trg_embed(trg), encoder_output))

    def decode_v(self, trg, encoder_output):
        return self.prediction_generator_v(self.decoder(self.trg_embed(trg), encoder_output))
...
def make_model(DEVICE, num_layers, encoder_input_size, decoder_output_size, d_model, adj_mx, nb_head, num_of_weeks,
    ...
    generator_m = nn.Linear(d_model, decoder_output_size)
    generator_v = nn.Linear(d_model, decoder_output_size)
