...
public class AccuracyTest_Interval {
    static int range = 1500;
    static int sampleInterval = 60;
    static int populationThreshold = 2;
    static double probabilityThreshold = 0.5;
    public static String result_true_interval = System.getProperty("user.dir") + "/result/result_true_interval_backup.txt";
    ...
    public static String outFileAcc = System.getProperty("user.dir") + "/result/" + "newHSM_interval_acc.csv";
    public static String outFileRecall = System.getProperty("user.dir") + "/result/" + "newHSM_interval_recall.csv";
    ...
    public static void getGroundTruth() throws IOException {
        ...
        try {
            FileWriter output = new FileWriter(result_true_interval, true);
            output.write(s);
            output.flush();
            output.close();
        } catch (IOException e) {
    ...
    public static void calAccuracy() throws IOException {
        String resultAcc = "interval" + "\t" + "exact" + "\t" + "appro" + "\n";
        String resultRecall = "interval" + "\t" + "exact" + "\t" + "appro" + "\n";

        Path path = Paths.get(result_true_interval);
        Scanner scanner = new Scanner(path);

        HashMap<Integer, HashMap<Integer, HashMap<Integer, ArrayList<Integer>>>> interval_true_map = new HashMap<>();
        while (scanner.hasNextLine()) {
            String line = scanner.nextLine();
            String [] tempArr1 = line.split("\t");
            int interval = Integer.parseInt(tempArr1[0]);
            ...
            interval_true_map.put(interval, tra_map);
        }

        Path path1 = Paths.get(result_pre_interval_exact);
        Scanner scanner1 = new Scanner(path1);

        HashMap<Integer, HashMap<Integer, HashMap<Integer, ArrayList<Integer>>>> interval_exact_map = new HashMap<>();
        while (scanner1.hasNextLine()) {
            ...
            interval_exact_map.put(interval, tra_map);
        }

        Path path2 = Paths.get(result_pre_interval_appro);
        Scanner scanner2 = new Scanner(path2);

        HashMap<Integer, HashMap<Integer, HashMap<Integer, ArrayList<Integer>>>> interval_appro_map = new HashMap<>();
        while (scanner2.hasNextLine()) {
            ...
            interval_appro_map.put(interval, tra_map);
        }

        ArrayList<Integer> intervals = new ArrayList<>(Arrays.asList(10, 60, 300));

        for (int interval: intervals) {
            resultAcc += interval;
            resultRecall += interval;
            ...
            HashMap<Integer, HashMap<Integer, ArrayList<Integer>>> tra_exact_map = interval_exact_map.get(interval);
            HashMap<Integer, HashMap<Integer, ArrayList<Integer>>> tra_true_map = interval_true_map.get(interval);
            HashMap<Integer, HashMap<Integer, ArrayList<Integer>>> tra_appro_map = interval_appro_map.get(interval);
            ...
            for (int i = 0; i < 10; i++) {
                ...
                for (int t = 0; t < DataGenConstant.endTime; t += interval) {
